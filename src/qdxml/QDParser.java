...
// No copyright claimed (see below)
// 
...
//
// The original source code for this file was taken from:
...
public class QDParser {
	// MAJ
	private static boolean sFinishNow = false;
  ...
  public static void parse(DocHandler doc,Reader r, int startPosition) throws Exception {
    ...
    while((c = r.read()) != -1) {
    	// MAJ
    	// Skip to prescribed start position.
    	if (startPosition > 0) {
    		--startPosition;
    		continue;
    	}
      ...
      // We are between tags collecting text.
    	  ...
    	  if(c == '<') {
        	...
        	if(sb.length() > 0) {
          	...
          	// Stay in text mode but return this string and start the next
    		  if(sb.length() > 0) {
        		doc.text(sb.toString());
        		sb.setLength(0);
    		  }
